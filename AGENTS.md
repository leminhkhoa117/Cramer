...
- `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL`, `OPENROUTER_API_KEY`, `OPENROUTER_GENERATION_MODEL`, `OPENROUTER_TIMEOUT_MS`
- `PAYOS_CLIENT_ID`, `PAYOS_API_KEY`, `PAYOS_CHECKSUM_KEY`, `PAYOS_RETURN_URL`, `PAYOS_CANCEL_URL`, `PAYOS_WEBHOOK_URL`
- `GEMINI_API_KEY` (speaking grading + Gemini Live), `SPEAKING_*` knobs (bank sizes, Lúa cost, grading tuning)
- `SERVER_PORT`, `DEBUG_ENABLED`, `DEBUG_SECRET_KEY`, `SPRING_JPA_SHOW_SQL`, `SWAGGER_ENABLED`

Frontend `.env`:

- `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`
- `VITE_API_BASE_URL` (optional, defaults to `/api`)
- `VITE_DEV_ADMIN_BYPASS` (dev only), `VITE_ADMIN_USER_IDS` (comma-separated extra admin IDs)
