...
public class PaymentServiceImpl implements PaymentService {
    ...
    private static final Map<Integer, Integer> LUA_PACKS = Map.of(
            100, 10000,    // Túi Lúa (100đ/Lúa)
            550, 45000,    // Gói Lúa (82đ/Lúa, 10% bonus)
            2500, 150000   // Xe Lúa (60đ/Lúa, 25% bonus)
    );
    ...
    private void processSubscriptionPayment(PaymentOrder order) {
        ...
        SubscriptionTier tier = tierRepository.findById(order.getTierId())
    ...
    public boolean verifyWebhookSignature(PayOSWebhookDTO webhook) {
        ...
        try {
            ...
            if (data.getAmount() != null) signData.put("amount", data.getAmount());
            if (webhook.getCode() != null) signData.put("code", webhook.getCode());
            if (data.getDescription() != null) signData.put("description", data.getDescription());
            if (data.getOrderCode() != null) signData.put("orderCode", data.getOrderCode());
            if (data.getPaymentLinkId() != null) signData.put("paymentLinkId", data.getPaymentLinkId());
            
            // Add other fields that PayOS includes
            if (data.getAccountNumber() != null) signData.put("accountNumber", data.getAccountNumber());
            if (data.getReference() != null) signData.put("reference", data.getReference());
            if (data.getTransactionDateTime() != null) signData.put("transactionDateTime", data.getTransactionDateTime());
    ...
    private String getLuaPackName(int amount) {
        return switch (amount) {
            ...
            case 550 -> "Gói Lúa (550)";
            case 2500 -> "Xe Lúa (2500)";
            default -> amount + " Lúa";
        };
    }
}
