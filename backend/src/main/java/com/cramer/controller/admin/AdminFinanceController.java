...
public class AdminFinanceController {
    ...
    public ResponseEntity<Map<String, Object>> getFinanceOverview(
        ...
        try {
            ...
            Long totalRevenue = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(SUM(amount_vnd), 0) FROM public.payment_orders WHERE status = 'PAID'"
                            + dateCondition,
                    Long.class);
            ...
            Long luaRevenue = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(SUM(amount_vnd), 0) FROM public.payment_orders WHERE status = 'PAID' AND type = 'LUA_PACK'"
                            + dateCondition,
                    Long.class);
            ...
            Long newSubscriptions = jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM public.payment_orders WHERE status = 'PAID' AND type = 'SUBSCRIPTION'"
                            + dateCondition,
                    Long.class);
            ...
            Long luaPacksSold = jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM public.payment_orders WHERE status = 'PAID' AND type = 'LUA_PACK'"
                            + dateCondition,
                    Long.class);
    ...
    /**
     * Helper method to get date condition based on period (with AND prefix for
     * overview queries)
     */
    private String getDateCondition(String period) {
        return switch (period) {
            ...
            case "yesterday" ->
                " AND paid_at >= DATE_TRUNC('day', NOW() - INTERVAL '1 day') AND paid_at < DATE_TRUNC('day', NOW())";
            ...
            case "lastMonth" ->
                " AND paid_at >= DATE_TRUNC('month', NOW() - INTERVAL '1 month') AND paid_at < DATE_TRUNC('month', NOW())";
    ...
    public ResponseEntity<Map<String, Object>> getReportsData(
        ...
        try {
            ...
            double totalChange = prevTotalRevenue != null && prevTotalRevenue > 0
                    ? ((totalRevenue - prevTotalRevenue) * 100.0 / prevTotalRevenue)
                    : 0;
            ...
            double subChange = prevSubRevenue != null && prevSubRevenue > 0
                    ? ((subscriptionRevenue - prevSubRevenue) * 100.0 / prevSubRevenue)
                    : 0;
            ...
            double luaChange = prevLuaRevenue != null && prevLuaRevenue > 0
                    ? ((luaRevenue - prevLuaRevenue) * 100.0 / prevLuaRevenue)
                    : 0;
            ...
            List<Map<String, Object>> chartData = jdbcTemplate.queryForList(Objects.requireNonNull(chartSql), dateFrom,
                    dateTo);
    ...
    public ResponseEntity<Map<String, Object>> getSubscriptionAnalysis(
        ...
        try {
            ...
            double churnRate = activeSubscribers != null && activeSubscribers > 0
                    ? ((cancelledSubscribers != null ? cancelledSubscribers : 0) * 100.0 / activeSubscribers)
                    : 0;
            ...
            try {
                ...
                totalSubCount = jdbcTemplate.queryForObject(
                        ...
                        Long.class);
                if (totalSubCount == null)
                    totalSubCount = 0L;
            } catch (Exception e) {
                log.warn("Could not calculate LTV: {}", e.getMessage());
            }
            Long ltv = (totalSubCount != null && totalSubCount > 0 && totalSubRevenue != null)
                    ? (totalSubRevenue / totalSubCount)
                    : 0;
            result.put("ltv", ltv);
    ...
    public ResponseEntity<Map<String, Object>> getLuaEconomy(
        ...
        try {
            ...
            // Try to get data from credit_transactions - use 'amount' column which is more
            // standard
    ...
    public ResponseEntity<Map<String, Object>> getUserAcquisition(
        ...
        try {
            ...
            double conversionRate = totalUsers != null && totalUsers > 0
                    ? ((paidUsers != null ? paidUsers : 0) * 100.0 / totalUsers)
                    : 0;
