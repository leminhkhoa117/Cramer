...
The cost of using custom provider keys on OpenRouter is a percentage of what the same model/provider would cost normally on OpenRouter and will be deducted from your OpenRouter credits.
This fee is waived for a certain number of BYOK requests per-month.
