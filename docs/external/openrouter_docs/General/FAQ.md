...
<AccordionGroup>
  <Accordion title="What are the fees for using OpenRouter?">
    OpenRouter charges a fee when you purchase credits. We pass through
    ...
    Crypto payments are charged a fee.
  </Accordion>
  ...
  <Accordion title="Is there a fee for using my own provider keys (BYOK)?">
    ...
    BYOK), the first requests per-month are free, and for all subsequent usage there is a fee
    of what the same model and provider would normally cost on OpenRouter. This fee is deducted
...
<AccordionGroup>
  ...
  <Accordion title="How are rate limits calculated?">
    ...
    If you have purchased a certain amount of credits, your free model rate limit will be higher.
    Otherwise, you will be rate limited to a lower number of free model API requests per day.
...
<AccordionGroup>
  ...
  <Accordion title="What free tier options exist?">
    ...
    and are usually not suitable for production use. If you have purchased a certain amount of credits,
    the free models will have higher limits.
  </Accordion>
