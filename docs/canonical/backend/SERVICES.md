...
**Total Services: 53** (Interface: 20, Concrete: 33)
...
| `NoopSpeakingEvaluationDispatchService` | Concrete | service.implement | No-op fallback when no grading worker is wired |
| `AdminSpeakingService` | Interface | service | Admin regrade with audit logging |
| `SpeakingSessionStatusTransitioner` | Concrete | service.implement | State machine: validates allowed status transitions |
| `SpeakingGradingWorker` | Concrete | service.implement | Core grading logic: audio prep, OpenRouter call, retry, persist |
| `SpeakingGradingWatchdog` | Concrete | service.implement | @Scheduled watchdog: fail-stuck-grading, re-enqueue-stuck-completed |
| `SpeakingGradingPromptBuilder` | Concrete | service.implement | Builds system prompt with IELTS band descriptors |
| `SpeakingAudioPreparer` | Concrete | service.implement | Downloads audio from Supabase Storage, transcodes via ffmpeg |
| `SpeakingEvaluationService` | Concrete | service.implement | Async dispatch via @Async("speakingGradingExecutor") |
| `SupabaseStorageService` | Concrete | service | Supabase Storage operations (download, upload, exists, getPublicUrl) |
...
| `SpeakingSelectionPlannerConfig` | config | Bean wiring — selects `LlmSpeakingSelectionPlannerService` or `HeuristicSpeakingSelectionPlannerService` |
| `SpeakingSessionProperties` | config | `speaking.session.*` props (lua cost, part bank sizes, defer-until-context) |
| `SpeakingGeminiLiveProperties` | config | `speaking.gemini.live.*` props (endpoint, model, sample rates, voice) |
| `SpeakingGradingAsyncConfig` | config | ThreadPoolTaskExecutor for async grading |
| `SpeakingEvaluationDispatchConfig` | config | Conditional fallback to NoopSpeakingEvaluationDispatchService |
...
- Grading watchdog runs every 60s via `@Scheduled`
