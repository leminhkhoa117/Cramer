...
| GET | `/course-history` | Get user course attempt history | Yes |
...
| GET | `/top-spenders` | Get top spending users |
| GET | `/export` | Export finance data |
...
**Base Path:** `/api/admin/speaking`

| Method | Path | Description | Auth |
|--------|------|-------------|------|
| POST | `/sessions/{id}/regrade` | Admin-triggered regrade of a Speaking session | Admin |

#### POST /api/admin/speaking/sessions/{id}/regrade

**Query Parameters:**

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `mode` | string | No | `multimodal` | Grading mode: `multimodal`, `text_only` |

**Notes:**
- Does not re-debit Lúa
- Logged with admin audit trail
